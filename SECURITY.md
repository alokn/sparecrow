...
- **Extended (v1.1, Story 27.3)**: The `~/.claude.json` file (startup metadata and cached feature gates) is now also mounted **unconditionally** read-only (`readonly: true`). Unlike the `~/.claude/` directory, this protection is NOT overridable by `mount_claude_config_readonly: false` — the asymmetry is intentional: `.claude.json` does not contain authentication credentials, so Claude Code tolerates write failures (they are non-fatal). This prevents a container process from corrupting host-side startup metadata or feature gate cache.
